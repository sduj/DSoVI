  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton(
        ...
        class = "info-icon", 
        `data-tooltip` = "Expected crisis period during which changes of population density and exposure will be assessed
"
      )),
      radioButtons("CrisisChoiceD", label = "",
                   choices = c("Daytime", "Evening", "Nightime"), selected = "Daytime",
                   inline = TRUE),
      radioButtons("CrisisChoiceW", label = "",
                   choices = c("Weekday", "Weekend"), selected = "Weekend",
                   inline = TRUE),
      ...
      div(class = "section-heading", "Reference",actionButton(
        ...
        label = icon("info-circle"), 
        class = "info-icon", 
        `data-tooltip` = "Reference time period serving as baseline for comparison to assess changes of population density and exposure
")),
      radioButtons("RefChoiceD", "",
                  choices = c("Daytime", "Evening", "Nightime"),selected = "Nightime",inline = TRUE),
      radioButtons("RefchoiceW", "",
                  choices = c("Weekday", "Weekend"),selected = "Weekend",inline = TRUE),
...
server <- function(input, output) {
  ...
  output$bivariatePlot <- renderPlot({
    ...
    plot_data <- expand_grid(
      Exposure = factor(1:3, labels = c("3", "2", "1")),
      Vulnerability = factor(1:3, labels = c("3", "2", "1"))
    ) %>%
